...
- node 단위부터 시작해 전체 구조를 스스로 설계하는 방향을 전제로 함
...
- 단순 사용보다 직접 구현 중심으로 접근
- 각 요소를 독립적으로 실험하고 이후 하나의 시스템으로 통합
- 추상 개념보다 실제 동작 가능한 모델을 우선
...
- 입력 신호와 내부 상태를 어떤 식으로 다룰지
- activation 단계에서 어떤 함수를 사용할지

### Consider

- activation을 단순 임계값 통과로 볼지
- 연속적인 출력 변환으로 볼지
- 시간 축이 있는 상태 변화와 연결할지

### Next

- voltage와 activation의 역할을 분리해서 정의
- 이후 activation function 후보를 비교
...
- voltage를 뉴런의 현재 상태로 본다면 시간 축이 중요해진다.
- voltage를 단순 합산값으로 본다면 일반적인 weighted sum과 가까워진다.
...
- 단일 layer perceptron
- 다층 feedforward network
- recurrent structure
- event-driven node network
- fully custom graph structure

### Consider

- node 연결을 고정된 layer로 볼지 그래프로 볼지
- feedback loop를 허용할지
- 시간 개념을 구조에 포함할지
- 실험용 최소 구조를 어디까지로 잡을지

### Next

- 가장 먼저는 단순 feedforward 구조 구현
- 이후 recurrent 또는 event-driven 확장 가능성 검토
...
- 능동 학습: 시스템이 스스로 탐색하고 상호작용하며 학습하는 방식
- 수동 학습: 주어진 데이터나 정답을 기반으로 학습하는 방식
...
- exploration을 얼마나 허용할지
- feedback를 어떤 형태로 받을지
- 실시간 처리와 어떻게 연결할지
- reward 또는 signal을 어떻게 정의할지

### Next

- 작은 환경에서 선택과 피드백 루프를 먼저 실험
- node 기반 구조와 결합 가능한지 확인
...
- supervised dataset을 사용할지
- 입력과 정답의 형식을 어떻게 정의할지
- batch 기반으로 처리할지 stream 기반으로 처리할지
- baseline 성능 측정에 적합한지

### Next

- 가장 단순한 입력-출력 매핑 문제부터 실험
- 이후 능동 학습과 비교 가능한 기준 만들기
