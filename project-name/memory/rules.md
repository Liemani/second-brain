...
- `requirements/`: 작업 전에 고정해야 하는 요구사항과 제약을 관리한다.
- `decisions/`: 시도와 판단을 바탕으로 내린 결정을 기록한다.
- `templates/`: 새 문서를 만들 때 사용하는 템플릿을 둔다.
- `extracted/`: 외부 문서를 text로 변환한 참고 자료를 보관한다. 작업 운영 문서라기보다 reference 영역이다.
...
5. 작업 전에 고정해야 할 요구사항이 있으면 `requirements/`에 정리한다.
6. 실제로 무언가를 해보면 `work_logs/`에 작업 진행 기록을 남긴다.
7. 시도 결과를 바탕으로 방향을 정하면 `decisions/`에 결정을 남긴다.
8. 필요하면 task에서 관련 todo, work log, requirement, decision을 링크한다.
...
- requirement는 관련 issue, task, decision을 가질 수 있다.
...
- `extracted/`는 작업 운영보다 참고 자료 보관에 가깝고, 필요할 때 issue / task / requirement / decision에서 링크해 사용한다.
- `wiki/`는 작업 운영보다 설명과 정리에 가깝고, 필요할 때 issue / task / requirement / decision에서 링크해 사용한다.
